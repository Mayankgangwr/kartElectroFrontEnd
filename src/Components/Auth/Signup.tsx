...
const Signup = observer(() => {
  const { UserRegister, loggingIn, Userlogin } = authViewModel;
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    const res1: any = await UserRegister(userDetails);
    if (res1) {
      const res: any = Userlogin(userDetails);
      if (res) {
        window.location.href = location?.state?.from ? location?.state?.from : "/";
      }
    }
            ...
            >
              ...
              <div className="w-full py-2   flex flex-col gap-4 items-center">
                <button
                  ...
                  disabled={isDisabled}
                >
                  {loggingIn ? "Signing up..." : "Create Account"}
                </button>
