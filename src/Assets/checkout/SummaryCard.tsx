...
const SummaryCard: React.FC<ISummaryCardProps> = observer(({ setShowModal }) => {
  return (
    <section className="py-3 md:py-7 px-5 md:px-7 lg:px-12 rounded-md shadow-sm bg-white/[0.7] flex flex-col gap-5 w-full h-min">
      ...
      {cartViewModel.allCartItems?.products.map((cartProduct) => {
        const item = productViewModel.products.find(el => el.id === cartProduct.productId);
        return item ? (
          <div
            className="flex flex-col gap-2 shadow-sm px-4 py-2 rounded-sm "
            key={cartViewModel.allCartItems?.id}
          >
            <div className="flex  items-center flex-wrap gap-2 w-full">
              ...
              <div className="text-lg">x{cartProduct.quantity}</div>
            </div>
          </div>
        ) : null;
      })}
      ...
      <div className="w-full py-2   flex gap-4 items-center">
        <button
          onClick={() => {
            if ([authViewModel.profile?.address].length === 0) {
